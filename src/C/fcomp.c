...
/*                -m     print matches                                       */
/*                -M<n>  seq print length for matches (default 20)           */
...
/*                -v     verbose output                                      */
/*                                                                           */
/* $Id: fcomp.c,v 1.1 2011/02/07 19:04:57 seanmccorkle Exp seanmccorkle $    */
/*                                                                           */
/*****************************************************************************/
...
#define MAX_PBUFF 60            /* print buffer max size */
...
int      print_len    = 20;   /* set by -M, strlen to print for -m option*/
int      hdr_trunc    = 10;
...
void  parse_args( int argc, char **argv )
   {
    ...
    while ( (c = getopt(argc, argv, "cdDmM:pt:s:T:v") ) != -1 )
        switch (c)
           {
            case 'c':
                       print_clust = 1;
            ...
            if ( print_matches )
               {
                ...
                if ( previous_above > 0 )
                    ...
                    printf( "          " );
                strncpy( pbuff, seq_buff + pos[j], print_len );
                pbuff[print_len] = '\0';
                printf( "  %s", pbuff );
                printf( "\n" );
               }
